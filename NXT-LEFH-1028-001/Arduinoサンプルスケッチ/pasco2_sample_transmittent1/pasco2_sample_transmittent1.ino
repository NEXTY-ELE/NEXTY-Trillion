...
// 測定間隔 10秒以上を指定すること、実際の測定は+1秒間隔となる
...
void setup() {
  ...
  // Idle mode
  i2c_write_byte(0x04, (0x00 | (CALIB_SETTING << 2)));  // MEAS_CFG
  ...
  /* 設定レジスタ値 */
  uint8_t meas_cfg = i2c_read_byte(0x04);
  Serial.print("meas mode :");
  Serial.print(meas_cfg, HEX);
  Serial.println();
  ...
  delay(1000);
  /* レジスタ設定0x04 */
  i2c_write_byte(0x04, 0x01 | (CALIB_SETTING << 2)); // Single-shot mode
  delay(2000);
...
void loop() {
  ...
  if(co2_int==true){
    ...
    digitalWrite(EN12V, LOW); // 12V OFF
    interrupts();
